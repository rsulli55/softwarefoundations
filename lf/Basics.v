...
Definition ltb (n m : nat) : bool :=
  if (leb m n) then false else true.
...
Proof.
  ...
  (* Q: Is there a way to avoid the destructuring of c in this case?
   { simpl.
  gives false = true -> c = true. It seems like there should be a way
...
(* Worked but gave a warning *)
(*
Fixpoint factorial' (n: nat) : nat :=
  if eqb n O then S O else n * factorial (pred n).
 *)
(* Failed *)
(*
Fixpoint added_factorial (n m : nat) : nat :=
  match n with
  | O => factorial m
  | S n' => added_factorial n (S m)
  end.
*)
(* FILL IN HERE
...
Proof.
  intros b c.
  destruct b eqn:Eb.
  - rewrite -> orb_commutative. rewrite -> orb_true_true1. simpl. intro H. rewrite -> H. reflexivity.
...
Theorem grade_lowered_once :
  forall (late_days : nat) (g : grade),
    (late_days <? 9 = false) ->
    (late_days <? 17 = true) ->
    (* Q: It seems like the theoem goes through with or without this hypothesis *)
    (grade_comparison (Grade F Minus) g = Lt) ->
