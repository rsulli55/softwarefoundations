...
Proof.
  ...
  - inversion H; subst.
    + apply E_IfFalse; try assumption; simpl.
      rewrite H5; reflexivity.
    + apply E_IfTrue; try assumption; simpl.
      rewrite H5; reflexivity.
  - Search (negb _ = _). inversion H; subst.
    + apply E_IfFalse; try assumption; simpl; simpl in H5;  rewrite <- negb_true_iff; assumption.     
    + apply E_IfTrue; try assumption; simpl; simpl in H5;  rewrite <- negb_false_iff; assumption.
Qed.
(** [] *)
...
Every while loop whosse guard is true will never terminate.
*)
(** [] *)
...
- exfalso. apply (while_true_nonterm _ c st st' Hb); assumption.
- assert (Htrue: bequiv <{true}> <{true}>). { unfold bequiv. intros; reflexivity. }
exfalso; apply (while_true_nonterm <{true}> <{skip}> st st' Htrue). assumption.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion Hseq. inversion H1. apply E_Seq with st'1; try assumption.
    + apply E_Seq with st'0; assumption.
  - inversion Hseq. inversion H4. apply E_Seq with st'1; try assumption.
    + apply E_Seq with st'0; assumption.
Qed.
(** [] *)
...
Theorem assign_aequiv : forall (X : string) (a : aexp),
  ...
  cequiv <{ skip }> <{ X := a }>.
Proof.
  intros X a Ha; unfold aequiv in Ha; split; intros H; inversion H.
  - assert (HX : st' =[ X := a ]=> (X !-> (aeval st' X) ; st')).
  { apply E_Asgn. rewrite Ha; reflexivity. }
  rewrite t_update_same in HX; assumption.
  - rewrite <- Ha in H4. rewrite <- H4. rewrite t_update_same.
  apply E_Skip.
Qed.
(** [] *)
...
Theorem CSeq_congruence : forall c1 c1' c2 c2',
  ...
  cequiv <{ c1;c2 }> <{ c1';c2' }>.
Proof.
  assert (Lem: forall c1 c1' c2 c2' st st',
  cequiv c1 c1' -> cequiv c2 c2' ->
  st =[ c1 ; c2 ]=> st' -> st =[ c1'; c2' ]=> st').
  {
    intros; unfold cequiv in *; inversion H1. apply E_Seq with st'0.
    + rewrite <- (H st st'0);  assumption.
    + rewrite <- (H0 st'0 st');  assumption.
  }
  intros; split.
  - apply Lem; assumption.
  - apply Lem; apply sym_cequiv; assumption.
Qed.
(** [] *)
...
Theorem CIf_congruence : forall b b' c1 c1' c2 c2',
  ...
  cequiv <{ if b then c1 else c2 end }>
         <{ if b' then c1' else c2' end }>.
Proof.
  assert (Lem:  forall b b' c1 c1' c2 c2' st st',
  bequiv b b' -> cequiv c1 c1' -> cequiv c2 c2' ->
  st =[ if b then c1 else c2 end ]=> st' -> 
  st =[ if b' then c1' else c2' end ]=> st').
  {
    intros; unfold cequiv, bequiv in *. inversion H2.
    - apply E_IfTrue; rewrite H, H0 in *; assumption. 
    - apply E_IfFalse; rewrite H, H1 in *; assumption.
  }
  intros; split.
  - apply Lem; assumption.
  - apply Lem; try apply sym_bequiv; try apply sym_cequiv; assumption.
Qed.
...
(* 
TODO: I'm having trouble coming up with relations on commands
*)
(* Do not modify the following line: *)
...
Proof.
  ...
  - (* BLe *)
    simpl.
    remember (fold_constants_aexp a1) as a1' eqn:Heqba1'.
    remember (fold_constants_aexp a2) as a2' eqn:Heqba2'.
    assert (Ha1: aeval st a1 = aeval st a1').
    { rewrite Heqba1'; apply fold_constants_aexp_sound. }
    assert (Ha2: aeval st a2 = aeval st a2').
    { rewrite Heqba2'; apply fold_constants_aexp_sound. }
    rewrite Ha1; rewrite Ha2. destruct a1'; destruct a2'; 
    try reflexivity.
    simpl; destruct (n <=? n0); reflexivity.
  - (* BGt *)
    simpl.
...
Fixpoint optimize_0plus_aexp (a : aexp) : aexp :=
  ...
  | ANum n => ANum n 
  | AId x => AId x 
  | APlus 0 a' => optimize_0plus_aexp a'
...
Proof.
  unfold atrans_sound. induction a; unfold aequiv; intros st;
  simpl;
  try reflexivity;
  try (rewrite IHa1; rewrite IHa2; reflexivity).
  -  destruct a1 eqn:Heqa1;
  try (rewrite IHa1; rewrite IHa2; reflexivity).
    + destruct n; simpl; try rewrite IHa1; rewrite IHa2; reflexivity.
Qed. 

Theorem optimize_0plus_bexp_sound :
  btrans_sound optimize_0plus_bexp.
Proof.
  unfold btrans_sound; induction b; unfold bequiv; intros st;
  simpl; 
  try reflexivity;
  try (rewrite IHb1; rewrite IHb2; reflexivity);
  try (rewrite IHb; reflexivity);
 try (rewrite <- optimize_0plus_aexp_sound, <- optimize_0plus_aexp_sound;
  reflexivity).
Qed.
...
Proof.
  unfold ctrans_sound; induction c; unfold cequiv;
  intros st st'; simpl; try reflexivity.
...
Proof.
  ...
  intros c st st'; unfold optimizer.
  assert (Hc: st =[ c ]=> st' <-> st =[ (fold_constants_com c) ]=> st' ). 
  { apply fold_constants_com_sound. }
  rewrite Hc; apply optimize_0plus_com_sound.
Qed.  
...
(** As we have seen (in theorem [ceval_deterministic] in the [Imp]
    ...
    to an uninitialized variable in a low-level language like C.  After   simpl; try reflexivity.
  - apply CAsgn_congruence; apply (optimize_0plus_aexp_sound fold_constants_aexp_sound).
    will run faster).
